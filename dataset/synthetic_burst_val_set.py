...
class SyntheticBurstValMy(torch.utils.data.Dataset):
    """ Synthetic burst validation set. The validation burst have been generated using the same synthetic pipeline as
    employed in SyntheticBurst dataset.
    """
    def __init__(self, root=None, initialize=True):
        ...
        args:
            ...
            initialize - boolean indicating whether to load the meta-data for the dataset
        """
        self.root_clean = "/hdd1/wrl/BurstSR/dataset/same_burst_clean_val"
        self.root_noisy = "/hdd1/wrl/BurstSR/dataset/same_burst_noisy_val"
        self.burst_list = list(range(300))
        self.burst_size = 14
    ...
    def _read_burst_image(self, root, index, image_id):
        im = cv2.imread('{}/bursts/{:04d}/im_raw_{:02d}.png'.format(root, index, image_id), cv2.IMREAD_UNCHANGED)
        im_t = torch.from_numpy(im.astype(np.float32)).permute(2, 0, 1).float() / (2**14)

        return im_t

    def _read_gt_image(self, root, index):
        gt = cv2.imread('{}/gt/{:04d}/im_rgb.png'.format(root, index), cv2.IMREAD_UNCHANGED)
        gt_t = (torch.from_numpy(gt.astype(np.float32)) / 2 ** 14).permute(2, 0, 1).float()
        return gt_t

    def _read_meta_info(self, root, index):
        with open('{}/gt/{:04d}/meta_info.pkl'.format(root, index), "rb") as input_file:
            meta_info = pkl.load(input_file)
    ...
    def __getitem__(self, index):
        ...
        """
        burst_name = '{:04d}'.format(index)
        burst = [self._read_burst_image(self.root_noisy,index, i) for i in range(self.burst_size)]
        burst = torch.stack(burst, 0)

        burst_clean = [self._read_burst_image(self.root_clean,index, i) for i in range(self.burst_size)]
        burst_clean = torch.stack(burst_clean, 0)

        gt = self._read_gt_image(self.root_noisy, index)
        meta_info = self._read_meta_info(self.root_noisy, index)
        meta_info['burst_name'] = burst_name
        return burst, gt, meta_info, burst_clean